...

{
    ...
    public function save_document_request(Request $request){
        ...
        $document_request=DocumentRequest::Create(['title'=>$request->title,'document_request_type_id'=>$request->document_request_type_id,'user_id'=>Auth::user()->id,'workflow_id'=>$workflow->id,'company_id'=>companyId(),'comments'=>$request->comment,'due_date'=>date('Y-m-d',strtotime($request->due_date))]);
